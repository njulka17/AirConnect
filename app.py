...
def book_tickets():
    ...
    data = request.get_json()
    flight = Flight.query.filter_by(id=data['flight_id'])
...
def add_flights():
    ...
    departure_time = data['departure_time'].split('T')
    d_yyyy, d_mm, d_dd = list(map(int, departure_time[0].split('-')))
    d_h, d_m = list(map(int, departure_time[1].split(':')))

    arrival_time = data['arrival_time'].split('T')
    a_yyyy, a_mm, a_dd = list(map(int, arrival_time[0].split('-')))
    a_h, a_m = list(map(int, arrival_time[1].split(':')))

    print(d_dd, d_mm, d_yyyy)
    print(a_dd, a_mm, a_yyyy)

    new_flight = Flight(
        flight_name=data['flight_name'],
        flight_number=data['flight_number'],
        departure_time=datetime.datetime(d_yyyy, d_mm, d_dd, d_h, d_m, 0),
        arrival_time=datetime.datetime(a_yyyy, a_mm, a_dd, a_h, a_m, 0),
        origin=data['origin'],
